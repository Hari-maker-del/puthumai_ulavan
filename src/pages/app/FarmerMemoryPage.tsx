...
export default function FarmerMemoryPage({ firstTimeOnboarding = false, onComplete }: { firstTimeOnboarding?: boolean; onComplete?: () => void }) {
  const { user } = useAuth();
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (firstTimeOnboarding) {
        const { error: metadataError } = await supabase.auth.updateUser({ data: { onboarding_completed: true, role: 'farmer', preferred_language: form.preferred_language ?? 'en' } });
  ...
  return (
    <div className="space-y-6">
      <PageHeader icon={Brain} title={firstTimeOnboarding ? 'Set Up Your Farm' : 'Farmer Memory'}
