...
## OpenWeather
Configure `VITE_DATA_GOV_API_KEY` in Vercel. Test valid key, invalid key, offline/network failure, cached response and empty/unavailable state.

## Gemini
Configure `GEMINI_API_KEY` as a server-side Vercel environment variable. Test normal prompt, invalid key, quota/429 and network failure. Never expose the key in source.
