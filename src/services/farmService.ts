...
function normalizeFarm(
  farm: Partial<FarmRecord> & Record<string, unknown>,
): FarmRecord {
  return {
    id: String(farm.id ?? ''),

    // Database uses user_id, while the existing app model uses owner_id.
    owner_id: farm.user_id
      ? String(farm.user_id)
      : farm.owner_id
        ? String(farm.owner_id)
        : undefined,
    ...
    district: farm.district
      ? String(farm.district)
      : undefined,

    irrigation_type: farm.irrigation_type
      ? String(farm.irrigation_type)
      : undefined,
...
export async function getFarms(
  ...
  return (data ?? []).map(farm =>
    normalizeFarm(farm as Partial<FarmRecord> & Record<string, unknown>),
  );
}
...
export async function createFarm(
  ...
  return normalizeFarm(
    data as Partial<FarmRecord> & Record<string, unknown>,
  );
}
