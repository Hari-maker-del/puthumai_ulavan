...
import { getMarketPrices } from '@/services/marketIntelligenceService';
...
function formatWeather(
  ...
  if (!weather) {
    return 'Weather data: UNAVAILABLE. Do not infer or invent current weather.';
  }
  ...
  const forecast = weather.forecast
    ...
    .map(
      (day) =>
        `${day.day}: ${day.condition}, ${day.tempLo}-${day.tempHi}°C, rain probability ${day.rainProbability ?? 'unknown'}%`,
    )
    .join('; ');

  return [
    `Weather source: live Open-Meteo data for ${weather.location}.`,
...
export async function buildCopilotContext(
  ...
  /*
   * Build the weather location safely.
   *
   * Priority:
   * 1. Farmer Memory district + state
   * 2. Farm location
   * 3. Farm district + state
   * 4. Empty string
   *
   * This also avoids mixing ?? and || without parentheses.
   */
  ...
  const farmLocation = firstFarm?.location
    ? String(firstFarm.location)
    : farmDistrictState;

  const weatherLocation =
    memoryLocation || farmLocation || '';

  const crop =
    memory?.current_crop ||
    String(firstFarm?.crop ?? '');

  const district =
    memory?.district ||
    (
      firstFarm?.district
        ? String(firstFarm.district)
        : null
    );
  ...
  const [
    ...
    weatherLocation
      ? fetchWeather(weatherLocation).catch(
          () => null,
        )
  ...
  const contextParts = [
    ...
    yieldAvailable
      ? `Yield information:\n${JSON.stringify({
          previous_yield_kg:
            memory?.previous_yield_kg ?? null,
          predictions:
            yieldRows.slice(0, 10),
        })}`
