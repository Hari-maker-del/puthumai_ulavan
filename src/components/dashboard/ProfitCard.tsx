...
export default function ProfitCard() {
  ...
  const profit = data?.finance ?? null;
  ...
  if (loading || !profit) {
    return (
      <Card padding="lg" className="h-full">
        <div className="text-sm text-ink-600 py-8 text-center">No profit data available yet.</div>
      </Card>
    );
  }

  const change = profit.revenueLastMonth ? ((profit.expectedProfit - profit.revenueLastMonth) / profit.revenueLastMonth) * 100 : 0;
  ...
  return (
    <Card padding="lg" className="h-full">
      ...
      <div className="mt-4 flex items-end gap-2">
        <span className="font-display font-bold text-3xl text-ink-900">₹{(profit.expectedProfit ?? 0).toLocaleString('en-IN')}</span>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between text-xs">
        <div>
          ...
          <div className="font-bold text-ink-900">{Math.round((profit.profitMargin ?? 0) * 100) / 100}%</div>
        </div>
        <div className="text-right">
          <div className="text-ink-600">Per acre</div>
          <div className="font-bold text-ink-900">₹{(profit.profitPerAcre ?? 0).toLocaleString('en-IN')}</div>
        </div>
      </div>
    </Card>
  );
