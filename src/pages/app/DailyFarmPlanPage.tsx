...
${context.assembled}`;
      ...
      setPlan(error instanceof Error ? error.message : 'Unable to generate the farm plan.');
      setSourceNote('The AI plan could not be generated. The safe checklist below is still available.');
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => { if (user?.id) void generate(); }, [user?.id]);
