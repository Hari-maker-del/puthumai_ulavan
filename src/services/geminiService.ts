 ...
 * The Gemini API key is intentionally never shipped to the browser.
 * Requests are authenticated through the user's Supabase access token and
 * proxied through /api/gemini where the secret key stays server-side.
 */
...
function buildHistory(seedMessages: ChatMessage[]): { role: 'user' | 'model'; text: string }[] {
  ...
  for (const message of seedMessages) {
    if (message.role === 'user') history.push({ role: 'user', text: message.text });
    if (message.role === 'assistant') history.push({ role: 'model', text: message.text });
  }
  return history;
}
...
export async function askGemini(prompt: string, farmerMemoryContext?: string, preferredLanguage?: string): Promise<string> {
  return callGemini({ mode: 'chat', prompt, history: [], farmerMemoryContext, preferredLanguage });
}
