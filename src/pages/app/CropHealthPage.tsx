...
export default function CropHealthPage() {
  ...
  const cropHealthTimeline = scansToShow
    .slice()
    .reverse()
    .map((scan) => ({
      date: scan.created_at ? new Date(scan.created_at).toLocaleDateString() : 'Recent',
      healthy: scan.status === 'Healthy' ? 100 : 0,
      action: scan.status === 'Healthy' ? 0 : 100,
    }));
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      ...
      toast(err instanceof Error ? err.message : 'Unable to load scan history', 'error');
    }
  }, [user?.id, toast]);
