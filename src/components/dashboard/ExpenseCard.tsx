...
export default function ExpenseCard() {
  const { user } = useAuth();
  const { data, loading } = useApiQuery(() => (user?.id ? expenseService.getExpenses(user.id) : Promise.resolve({ rows: [], total: 0, byCategory: [] })), Boolean(user?.id));

  const total = data?.total ?? 0;
  const expenseByCategory = data?.byCategory ?? [];
  ...
  return (
    <GlassCard padding="lg" className="h-full flex flex-col">
      ...
      <div className="mt-4 flex items-end gap-2">
        <span className="font-display font-extrabold text-3xl text-ink-900">₹{(total ?? 0).toLocaleString('en-IN')}</span>
      ...
      <div className="mt-4 flex items-center gap-4">
        ...
        <div className="flex-1 space-y-1.5">
          {loading ? (
            ...
            <div className="text-sm text-ink-600">No expenses recorded yet.</div>
          )}
        </div>
      </div>
    </GlassCard>
